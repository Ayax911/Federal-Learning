#!/bin/bash
# Post-hoc evaluation: exp31 (centralized), exp28/29/30 (FedAvg/FedYogi/FedProx)
# Con predicciones y evaluaciones por nodos para los federados
...
{
  echo "════════════════════════════════════════════════════════════════════════════════"
  echo "  POST-HOC EVALUATION: exp31 (centralized), exp28/29/30 (FedAvg/FedYogi/FedProx)"
  ...
  experiments=(
    "exp31:exp31_centralized_radimagenet:runs/exp31_centralized_radimagenet/weights/final.pt"
    "exp28:exp28_fedavg_radimagenet:runs/exp28_fedavg_radimagenet/weights/global_model.pt"
    "exp29:exp29_fedyogi_radimagenet:runs/exp29_fedyogi_radimagenet/weights/global_model.pt"
    "exp30:exp30_fedprox_radimagenet:runs/exp30_fedprox_radimagenet/weights/global_model.pt"
  )
...
SHOW_METRICS
  ...
  for exp_spec in "${experiments[@]}"; do
    ...
    # Per-node evaluations (only for federated: exp28, exp29, exp30 — not exp31 centralized)
    if [ "$exp_short" != "exp31" ]; then
  ...
  for exp_full in exp31_centralized_radimagenet exp28_fedavg_radimagenet exp29_fedyogi_radimagenet exp30_fedprox_radimagenet; do
    echo "📊 $exp_full"
...
SUMMARY
    ...
    if [[ "$exp_full" == *"fedavg"* ]] || [[ "$exp_full" == *"fedyogi"* ]] || [[ "$exp_full" == *"fedprox"* ]]; then
      for node in 1 2 3 4 5; do
