...
**Six config generators live in `scripts/`; none of their output should be hand-edited.**
...
Block 8 once more on the 70/20/10 manifest); `gen_federated_grid.py` emits the federated YAML grid (8 files per experiment:
...
Not yet executed as of 2026-08-25 — no `runs/exp4{2,3,4,5,6}...`. Everything is held fixed against
...
`gen_split702010_block_grid.py` off of `exp42`.
